        ...
        try:
            ...
            if flag == True:    
                ...
                if last_chat_text.lower() == 'score' or last_chat_text.lower() == '/score':
                    fixtures = data_bot.get_fix(datetime.now()-timedelta(days=7), datetime.now()+timedelta(days=1))
                    text = ''
            ...
            fixtures = data_bot.get_fix(datetime.now()-timedelta(days=2), datetime.now()+timedelta(days=1))
            # goal update
            text = ''
            for fixture in fixtures:
                if (fixture['status'] == 'IN_PLAY' or fixture['status'] == 'PAUSED') and fixture['competition']['id'] in codes:
